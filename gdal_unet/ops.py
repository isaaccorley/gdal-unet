...
GDALNN = str(ROOT / "cpp" / "build" / "gdalnn_conv")
NTH = int(os.environ.get("GDALNN_THREADS", "16"))

# Auto-augment LD_LIBRARY_PATH with the ftw-tile conda env if it exists, so
# subprocesses can find libgdal/libstdc++ even when the parent Python is the
# torchgeo env.  No-op if the env is already correct.
_FTW = "/projects/bgtj/isaaccorley/envs/ftw-tile/lib"
if os.path.isdir(_FTW):
    _ld = os.environ.get("LD_LIBRARY_PATH", "")
    if _FTW not in _ld.split(":"):
        os.environ["LD_LIBRARY_PATH"] = f"{_FTW}:{_ld}" if _ld else _FTW
